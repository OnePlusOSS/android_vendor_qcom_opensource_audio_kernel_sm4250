	...
	struct sia81xx_dev_s *sia81xx) 
{
	...
	if (AUDIO_SCENE_RECEIVER == sia81xx->scene) {
		pr_debug("[debug][%s] %s:enter receiver mode \r\n", 
		LOG_FLAG, __func__);
		regmap_write(sia81xx->regmap, 0x00, 0x00);
		regmap_write(sia81xx->regmap, 0x00, 0x04);
		regmap_write(sia81xx->regmap, 0x00, 0x06);
		regmap_write(sia81xx->regmap, 0x04, 0x02);		
	} else {
